...
/**
 * SecretManager - Encrypt và decrypt sensitive data (passwords, tokens)
 * Sử dụng AES-256-CBC encryption
 * 
 * @class
 * @example
 * const secretManager = new SecretManager();
 * const encrypted = secretManager.encrypt('my-password');
...
class SecretManager {
  constructor() {
    ...
    if (!keyHex) {
      console.warn('[SecretManager] ENCRYPTION_KEY not set in .env - Using default key (NOT SECURE for production!)');
      ...
      try {
        ...
        console.error('[SecretManager] Invalid ENCRYPTION_KEY format:', error.message);
        console.error('[SecretManager] Generate key with: node -e "console.log(require(\'crypto\').randomBytes(32).toString(\'hex\'))"');
        throw error;
      }
    }
  }
  ...
  decrypt(encryptedText) {
    ...
    try {
      ...
      if (!encryptedText.includes(':')) {
        ...
        console.warn('[SecretManager] Decrypting plain text (not encrypted) - consider re-encrypting');
...
/**
 * Get SecretManager singleton instance
